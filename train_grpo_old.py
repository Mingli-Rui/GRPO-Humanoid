...
if __name__ == "__main__":
    ...
    try:
        group_size = 1
        for epoch_group in range(args.n_epochs // group_size):
            # Create Buffer group
            buffer_group = BufferGroup(group_size)
            ...
            for _ in range(0, group_size):
                ...
                for step_idx in range(0, args.n_steps):
                    ...
                    with torch.no_grad():
                        ...
                        actions, logprobs, _, _ = agent.get_action_and_value(obs)
                        # values = values.flatten()
                ...
                # Add collected trajectory to the group
                buffer_group.add(buffer)

            # After collecting trajectories,
            for group_idx in range(0, group_size):
                buffer, traj_adv, traj_ret = buffer_group.get_advantage(group_idx)
                reward_list = buffer.get_rewards_list()
                ...
                for _ in range(args.train_iters):
                    ...
                    for start_idx in range(0, args.n_steps, args.batch_size):
                        ...
                        _, new_logprobs, entropies, _ = agent.get_action_and_value(traj_obs[batch_indices],
                                                                                   traj_act[batch_indices])
                ...
                epoch = epoch_group * group_size + group_idx + 1 # 1-based index
